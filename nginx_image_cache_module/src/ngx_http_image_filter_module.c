...
ngx_http_image_resize(ngx_http_request_t *r, ngx_http_image_filter_ctx_t *ctx)
{
    ...
    if((conf->save_as_webp && (ctx->type == NGX_HTTP_IMAGE_JPEG || ctx->type == NGX_HTTP_IMAGE_PNG)) 
        ...
        if(conf->no_resize == 0 && ctx->no_resize == 0) {
...
ngx_http_image_filter_merge_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_value(conf->save_as_webp, prev->save_as_webp, 0);
    ngx_conf_merge_value(conf->no_resize, prev->no_resize, 0);
