...
int ReadJPEG(unsigned char * const inbuffer, unsigned long const insize, WebPPicture* const pic, Metadata* const metadata) {
  int ok = 0;
 ...
 Error:
  ...
  jpeg_mem_src((j_decompress_ptr)&dinfo, inbuffer, insize);
...
#else  // !WEBP_HAVE_JPEG
int ReadJPEG(unsigned char * const inbuffer, unsigned long const insize, struct WebPPicture* const pic,
