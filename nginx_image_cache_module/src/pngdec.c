...
int ReadPNG(unsigned char* data, const unsigned int data_size, struct WebPPicture* const pic, int keep_alpha,struct Metadata* const metadata) 
{
 ...
 Error:
  ...
  imgsource.data = data;
  imgsource.size = data_size;
  imgsource.offset = 0;
  png_set_read_fn(png, &imgsource,pngReadCallback);
  //png_read_png(png, info, PNG_TRANSFORM_EXPAND | PNG_TRANSFORM_STRIP_ALPHA, 0);
